...
namespace TransferManagerCE
{
    internal class SelectionTool : DefaultTool
    {
        ...
        private void HighlightNodes(RenderManager.CameraInfo cameraInfo)
        {
            ...
            if (iShowConnection > 0)
            {
                ...
                switch (iShowConnection)
                {
                    ...
                    case 2:
                        {
                            connectionNodes = UnconnectedGraphCache.GetPedestrianZoneServicesBufferCopy();
                            break;
                        }
                    case 3:
                        {
                            connectionNodes = UnconnectedGraphCache.GetOtherServicesBufferCopy();
                            break;
                        }
                }
        ...
        {
            if (m_color == null || m_color.Length < iColors)
            {
