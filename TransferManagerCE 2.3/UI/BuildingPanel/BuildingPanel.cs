...
namespace TransferManagerCE.UI
{
    public class BuildingPanel : UIPanel
    {
        ...
        private void OnReset(UIComponent component, UIMouseEventParameter eventParam)
        {
            ...
            {
                PathFindFailure.ResetPathingStatistics(m_subBuildingId);
            }
            UpdatePanel();
        }
        ...
        public List<PathingContainer> GetPathingIssues()
        {
            ...
            InstanceID subInstance = new InstanceID { Building = m_subBuildingId };
            ...
            foreach (KeyValuePair<Util.PATHFINDPAIR, long> kvp in failures)
            {
                ...
                {
                    list.Add(new PathingContainer(kvp.Value, kvp.Key.m_source, kvp.Key.m_target, 1, false));
                }
                else if (kvp.Key.m_target == instance)
                {
                    list.Add(new PathingContainer(kvp.Value, kvp.Key.m_source, kvp.Key.m_target, 2, false));
                }
                // Check sub building as well
                else if (m_subBuildingId != 0)
                {
                    if (kvp.Key.m_source == subInstance)
                    {
                        list.Add(new PathingContainer(kvp.Value, kvp.Key.m_source, kvp.Key.m_target, 1, true));
                    }
                    else if (kvp.Key.m_target == subInstance)
                    {
                        list.Add(new PathingContainer(kvp.Value, kvp.Key.m_source, kvp.Key.m_target, 2, true));
                    }
                }
            }
