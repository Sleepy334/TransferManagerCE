...
namespace TransferManagerCE
{
    public class PathDistance
    {
        ...
        private HashSet<ushort>? m_candidates = null;
        ...
        public uint FindNearestNeighbor(bool bstartActive, ushort startNodeId, HashSet<ushort>? candidates, NetInfo.LaneType laneType)
        {
            ...
            if (candidates != null && candidates.Count > 0)
            {
                ...
                while (m_sortedNodes.Count > 0)
                {
                    ...
                    ProcessNode(usMinNodeId, bstartActive, laneType);
        ...
        private void ProcessNode(ushort nodeId, bool bStartActive, NetInfo.LaneType laneType)
        {
            ...
            if (node.m_flags != 0 && (node.Info.m_laneTypes & laneType) != 0)
            {
                ...
                for (int i = 0; i < 8; ++i)
                {
                    ...
                    if (segmentId != 0)
                    {
                        ProcessSegment(segmentId, nodeId, bStartActive, fCurrentTravelTime, laneType);
                    }
                }
                ...
                while (laneId != 0)
                {
                    ...
                    if (lane.m_flags != 0)
                    {
                        if (lane.m_segment != 0)
                        {
                            ProcessSegment(lane.m_segment, nodeId, bStartActive, fCurrentTravelTime, laneType);
                        }
                    }
        ...
        private void ProcessSegment(ushort segmentId, uint uiCurrentNodeId, bool bStartActive, float fCurrentTravelTime, NetInfo.LaneType laneType)
        {
            if (segmentId != 0)
            {
                ...
                {      
                    ...
                    float fTravelTime = UncongestedTravelTime(segmentId, segment, direction, laneType);
                    if (fTravelTime > 0)
                    {
                        ...
                        while (laneId != 0)
                        {
                            ...
                            if (lane.m_flags != 0)
                            {
                                ...
                                {
                                    NetNode node = NetNodes.m_buffer[nodeId];
                                    if ((node.Info.m_laneTypes & laneType) != 0)
                                    {
        ...
        private float UncongestedTravelTime(ushort segmentId, NetSegment segment, NetInfo.Direction direction, NetInfo.LaneType laneTypes)
        {
