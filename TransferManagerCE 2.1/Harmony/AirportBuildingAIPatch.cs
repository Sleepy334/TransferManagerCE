    ...
    public static class AirportBuildingAIPatch
    {
        ...
        public static void HandleCrime(ushort buildingID, ref Building data, int crimeAccumulation, int citizenCount)
        {
            if (SaveGameSettings.GetSettings().EnableNewTransferManager)
            {
                ...
                if (citizenCount > 0 && crimeBuffer > citizenCount * 15 && Singleton<UnlockManager>.instance.Unlocked(ItemClass.Service.PoliceDepartment) && Singleton<SimulationManager>.instance.m_randomizer.Int32(5U) == 0)
                {
#if DEBUG
