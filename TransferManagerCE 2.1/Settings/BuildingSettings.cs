    ...
    public class BuildingSettings
    {
        ...
        public bool ReleaseBuilding(ushort buildingId)
        {
            ...
            foreach (KeyValuePair<int, RestrictionSettings> kvp in m_restrictions)
            {
                ...
                if (settings.m_incomingBuildingsAllowed.Contains(buildingId))
                {
                    bChanged = true;
                    settings.m_incomingBuildingsAllowed.Remove(buildingId);
                }
                if (settings.m_outgoingBuildingsAllowed.Contains(buildingId))
                {
                    bChanged = true;
                    settings.m_outgoingBuildingsAllowed.Remove(buildingId);
                }
            }
            return bChanged;
        }
