    ...
    public class OutsideConnectionPanel : UIPanel
    {
        ...
        public override void Start()
        {
            ...
            eventPositionChanged += (sender, e) =>
            {
                ...
                settings.OutsideConnectionLocationSaved = true;
                settings.OutsideConnectionLocation = absolutePosition;
                settings.Save();
            };
            if (ModSettings.GetSettings().OutsideConnectionLocationSaved)
            {
                absolutePosition = ModSettings.GetSettings().OutsideConnectionLocation;
                FitToScreen();
            }
            else
            {
            ...
            // Issue list
            m_listConnections = ListView.Create(this, "ScrollbarTrack", 0.8f, width - 20f, height - m_title.height - 10);
            if (m_listConnections != null)
            {
                ...
                m_listConnections.AddColumn(ListViewRowComparer.Columns.COLUMN_IMPORT, Localization.Get("listConnectionImport"), "Import", iCOLUMN_WIDTH_SMALL, iHEADER_HEIGHT, UIHorizontalAlignment.Center, UIAlignAnchor.TopLeft, null);
                m_listConnections.AddColumn(ListViewRowComparer.Columns.COLUMN_OWN, Localization.Get("listConnectionOwn"), "Own Vehicles", iCOLUMN_WIDTH_SMALL, iHEADER_HEIGHT, UIHorizontalAlignment.Center, UIAlignAnchor.TopLeft, null);
                m_listConnections.AddColumn(ListViewRowComparer.Columns.COLUMN_EXPORT, Localization.Get("listConnectionExport"), "Export", iCOLUMN_WIDTH_SMALL, iHEADER_HEIGHT, UIHorizontalAlignment.Center, UIAlignAnchor.TopLeft, null);
                m_listConnections.AddColumn(ListViewRowComparer.Columns.COLUMN_GUEST, Localization.Get("listConnectionGuest"), "Guest Vehicles", iCOLUMN_WIDTH_SMALL, iHEADER_HEIGHT, UIHorizontalAlignment.Center, UIAlignAnchor.TopLeft, null);
            }
        ...
        public void UpdatePanel()
        {
            if (m_listConnections != null)
            {
                ...
                connections.Sort();
                m_listConnections.SetItems(connections.ToArray());
            }
        }
    }
}