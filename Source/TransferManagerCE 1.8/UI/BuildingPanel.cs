...
namespace TransferManagerCE
{
    public class BuildingPanel : UIPanel
    {
        ...
        public override void Start()
        {
            ...
            if (tabVehicles != null)
            {
                ...
                if (m_listVehicles != null)
                {
                    ...
                    m_listVehicles.AddColumn(ListViewRowComparer.Columns.COLUMN_VALUE, Localization.Get("listBuildingPanelStatusColumn2"), "Vehicle transfer value", iCOLUMN_WIDTH_NORMAL, BuildingPanel.iHEADER_HEIGHT, UIHorizontalAlignment.Center, UIAlignAnchor.TopLeft, null);
            ...
            if (tabTransfers != null)
            {
                ...
                if (m_listMatches != null)
                {
                    ...
                    m_listMatches.AddColumn(ListViewRowComparer.Columns.COLUMN_ACTIVE, Localization.Get("listBuildingPanelMatchesColumn3"), "Active or Passive for this match", iCOLUMN_WIDTH_SMALL, BuildingPanel.iHEADER_HEIGHT, UIHorizontalAlignment.Left, UIAlignAnchor.TopLeft, null);
        ...
        {
            TransferManagerCEThreading.ShowTransferIssuePanel();
        }
        ...
        public List<PathingContainer> GetPathingIssues()
        {
            ...
            {
                if (kvp.Key.sourceBuilding == m_buildingId || kvp.Key.targetBuilding == m_buildingId)
                {
            ...
            if (m_buildingId != 0)
            {
                ...
                if (m_tabStrip != null)
                {
                    ...
                    if (m_tabStrip.IsTabVisible((int) TabIndex.TAB_VEHICLES))
                    {
                        ...
                        m_tabStrip.SetTabText((int)TabIndex.TAB_VEHICLES, Localization.Get("tabBuildingPanelVehicles") + "(" + vehicles.Count + ")");
                    } else
                    {
                    ...
                    {
                        m_tabStrip.SetTabText((int)TabIndex.TAB_STATUS, Localization.Get("tabBuildingPanelStatus") + "(" + CitiesUtils.GetGuestParentVehiclesForBuilding(m_buildingId).Count + ")");
                    }

                    // Update pathing tab count
                    List<PathingContainer> list = GetPathingIssues();
                    m_tabStrip.SetTabVisible((int)TabIndex.TAB_PATHING, list.Count > 0);
                        ...
                        case TabIndex.TAB_STATUS: // Status
                            {
                                if (m_listStatus != null)
                                {
                                    List<StatusContainer> status = new StatusHelper().GetStatusList(m_buildingId);
                                    status.Sort();
                                    m_listStatus.SetItems(status.ToArray());
                                }
                                break;
                            }
                        ...
                        case TabIndex.TAB_TRANSFERS: // Transfers
                            {
                                ...
                                {
                                    List<MatchData> listMatches = MatchLogging.instance.GetMatchesForBuilding(m_buildingId);
                                    if (listMatches != null)
                                    {
                                        listMatches.Sort();
                                        m_listMatches.SetItems(listMatches.Take(50).ToArray());
                                    }
                                }
                                break;
                            }
