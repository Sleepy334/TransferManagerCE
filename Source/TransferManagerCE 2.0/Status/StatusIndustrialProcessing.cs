...
namespace TransferManagerCE.Data
{
    public class StatusIndustrialProcessing : StatusData
    {
        ...
        public override string GetValue()
        {
            ...
            {
                if (m_material == GetIncomingTransferReason(m_buildingId) || m_material == GetSecondaryIncomingTransferReason(m_buildingId))
                {
                    int iMaxIncomingLoadSize = 4000;
                    ...
                    int consumptionDivider = GetConsumptionDivider(building);
                    int iStorageCapacity = Mathf.Max(iProductionCapacity * 500 / consumptionDivider, iMaxIncomingLoadSize * 4);
                    return Math.Round((double)building.m_customBuffer1 * 0.001, 1) + "/" + ((double)iStorageCapacity * 0.001);
                }
                else if (m_material == GetOutgoingTransferReason(building))
                {
                    int iProductionCapacity = processingAI.CalculateProductionCapacity((ItemClass.Level)building.m_level, new Randomizer(m_buildingId), building.Width, building.Length);
