    ...
    internal class OutsideConnectionSettings
    {
        ...
        public string GetName(ushort buildingId)
        {
            if (string.IsNullOrEmpty(m_name))
            {
                ...
                return CitiesUtils.GetBuildingName(buildingId) + " #" + iPosition;
            }
            else
            {
                return m_name;
        ...
        {
            if (iGlobalVersion >= 14 && Data != null && Data.Length > iIndex)
            {
                int iOutsideConnectionSettingsVersion = StorageData.ReadInt32(Data, ref iIndex);
#if DEBUG
                DebugLog.LogInfo("Global: " + iGlobalVersion + " OutsideConnectionSettingsVersion: " + iOutsideConnectionSettingsVersion + " DataLength: " + Data.Length + " Index: " + iIndex);
#endif
                ...
                if (iOutsideConnectionSettingsVersion <= iSETTINGS_DATA_VERSION)
                {
                    ...
                    for (int i = 0; i < iCount; ++i)
                    {
                        ...
                        if (settings != null && !settings.Equals(GetDefaultSettings((ushort) buildingId)))
                        {
