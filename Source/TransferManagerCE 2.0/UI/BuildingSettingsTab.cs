    ...
    public class BuildingSettingsTab
    {
        ...
        public void UpdateSettingsTab()
        {
            ...
            if (CanShowSettingsTab())
            {
                ...
                if (m_grpDistrictRestrictions != null)
                {
                    HashSet<TransferReason> incomingReasons = DistrictRestrictions.IncomingBuildingDistrictRestrictionsSupported(eType, m_buildingId);
                    HashSet<TransferReason> outgoingReasons = DistrictRestrictions.OutgoingBuildingDistrictRestrictionsSupported(eType, m_buildingId);
                    if (incomingReasons.Count > 0 || outgoingReasons.Count > 0)
                    {
                        ...
                        {
                            m_panelIncomingDistrict.isVisible = incomingReasons.Count > 0;

                            if (m_dropPreferLocalIncoming != null)
                            {
                                m_dropPreferLocalIncoming.isVisible = incomingReasons.Count > 0;
                                m_dropPreferLocalIncoming.selectedIndex = (int)settings.m_iPreferLocalDistrictsIncoming;
                                m_dropPreferLocalIncoming.tooltip = GetRestrictionReasons(incomingReasons);
                            }
                            if (m_btnIncomingSelectDistrict != null)
                            {
                                m_btnIncomingSelectDistrict.isVisible = incomingReasons.Count > 0;
                        ...
                        {
                            m_panelOutgoingDistrict.isVisible = outgoingReasons.Count > 0;

                            if (m_dropPreferLocalOutgoing != null)
                            {
                                m_dropPreferLocalOutgoing.isVisible = outgoingReasons.Count > 0;
                                m_dropPreferLocalOutgoing.selectedIndex = (int)settings.m_iPreferLocalDistrictsOutgoing;
                                m_dropPreferLocalOutgoing.tooltip = GetRestrictionReasons(outgoingReasons);
                            }
                            if (m_btnOutgoingSelectDistrict != null)
                            {
                                m_btnOutgoingSelectDistrict.isVisible = outgoingReasons.Count > 0;
                        ...
                        else if (incomingReasons.Count > 0)
                        {
                            m_grpDistrictRestrictions.height = 60;
                        }
                        else if (outgoingReasons.Count > 0)
                        {
                ...
                if (m_panelServiceDistance != null)
                {
                    HashSet<TransferReason> incomingReasons = IncomingDistanceRestrictionSupported(eType, m_buildingId);
                    HashSet<TransferReason> outgoingReasons = OutgoingDistanceRestrictionSupported(eType, m_buildingId);
                    if (incomingReasons.Count > 0 || outgoingReasons.Count > 0)
                    {
                        ...
                        if (m_sliderServiceDistance != null)
                        {
                            ...
                            m_panelServiceDistance.tooltip = GetDistanceReasons(incomingReasons, outgoingReasons);
                ...
                if (m_buttonGroup != null)
                {
                    ...
                    m_buttonGroup.Text = Localization.Get("GROUP_BUILDINGPANEL_APPLYTOALL") + ": " + sTypeDescription;
                }
            }
            else
            {
        ...
        private string GetDistanceReasons(HashSet<TransferReason> incoming, HashSet<TransferReason> outgoing)
        {
            string sReasons = "";

            // Warehouses, just return the actual material they store
            CustomTransferReason actualTransferReason = TransferReason.None;
            Building building = BuildingManager.instance.m_buildings.m_buffer[m_buildingId];
            WarehouseAI? warehouseAI = building.Info.GetAI() as WarehouseAI;
            if (warehouseAI != null)
            {
                actualTransferReason = warehouseAI.GetActualTransferReason(m_buildingId, ref building);
            }

            if (incoming.Count > 0)
            {
                sReasons += "Incoming Reasons:";
                if (actualTransferReason != TransferReason.None && incoming.Contains(actualTransferReason))
                {
                    sReasons += $"\r\n{actualTransferReason}";
                }
                else
                {
                    foreach (CustomTransferReason reason in incoming)
                    {
                        sReasons += $"\r\n{reason}";
                    }
                }
            ...
            if (outgoing.Count > 0)
            {
                ...
                sReasons += "Outgoing Reasons:";
                if (actualTransferReason != TransferReason.None && outgoing.Contains(actualTransferReason))
                {
                    sReasons += $"\r\n{actualTransferReason}";
                }
                else
                {
                    foreach (CustomTransferReason reason in outgoing)
                    {
                        sReasons += $"\r\n{reason}";
                    }
                }
