...
namespace SleepyCommon
{
    public class Utils
    {
        ...
        public static string DisplayTicks(long ticks)
        {
            return ((double)ticks * 0.0001).ToString("F");
        }

        public static string MakePercent(double dCurrent, double dMax, int iDecimals = 1)
        {
            if (dMax == 0.0)
            {
                return "0%";
            }
            return $"{(dCurrent / dMax * 100.0).ToString($"N{iDecimals}")}%";
        }

        public static string AddStringsWithNewLine(string string1, string string2)
        {
            if (string1.Length > 0)
            {
                return $"{string1}\n{string2}";
