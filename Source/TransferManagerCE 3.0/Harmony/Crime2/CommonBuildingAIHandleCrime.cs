...
namespace TransferManagerCE
{
    ...
    public class CommonBuildingAIHandleCrime
    {
        ...
        public static void PatchCrime2Handler()
        {
            if (DependencyUtils.IsNaturalDisastersDLC())
            {
                if (SaveGameSettings.GetSettings().EnableNewTransferManager)
                {
                    ...
                    {
#if DEBUG
                        Debug.Log("Patching Crime2 handler...", false);
#endif
                ...
                {
#if DEBUG
                    Debug.Log("Unpatching Crime2 handler...", false);
#endif
        ...
        public static IEnumerable<CodeInstruction> HandleCrimeTranspiler(ILGenerator generator, IEnumerable<CodeInstruction> instructions)
        {
            ...
            if (s_bPatched)
            {
                Debug.Log($"ERROR: CommonBuildingAI.HandleCrime - Already patched!", false);
            ...
            {
                Debug.Log("HandleCrimeTranspiler - Patching of CommonBuildingAI.HandleCrime succeeded", false);
            }
            else
            {
                Debug.Log($"HandleCrimeTranspiler - Patching of CommonBuildingAI.HandleCrime failed. bAddedBranch: {bAddedBranch} bAddedLabel: {bAddedLabel} bAddedAddOffers: {bAddedAddOffers}", false);
            }
        }
